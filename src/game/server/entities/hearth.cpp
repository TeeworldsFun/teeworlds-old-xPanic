	...
	if (distance(pTarget->m_Pos, m_Pos) < pTarget->m_ProximityRadius + 2.0f && GameServer()->m_apPlayers[m_Owner]->m_LifeActives)
	{
		if (pTarget && pTarget->GetPlayer())
			if(pTarget->HeartShield)
			{
				...
				GameServer()->CreateSound(pTarget->m_Pos+vec2(30, 0), 6);
				pTarget->SwitchHeartShield(0);
				char aBuf[64];
				str_format(aBuf, sizeof(aBuf), "%s literally eaten the heart! LOL!", Server()->ClientName(pTarget->GetPlayer()->GetCID()));
				GameServer()->SendChatTarget(-1, aBuf);
			}
			else
			{
				GameServer()->CreateSound(pTarget->m_Pos, 38);
