...
IGameController::IGameController(class CGameContext *pGameServer)
{
	...
	for(int i = 0; i < 48; i++)
	{
		...
		m_Door[i].m_TimeToOpen = 10; // для таймера
		m_Door[i].m_TickWhenOpening = 0; // сохраняет в себя тик во время касания тайла, чтобы потом от него плясать с непосредственно таймepa
	}
}
...
void IGameController::OnCharacterSpawn(class CCharacter *pChr)
{
	...
	if (GameServer()->m_apPlayers[BigLvlID] && pChr->GetPlayer()->GetCID() != BigLvlID)
	{
		...
		{
			int someint = 700 + pChr->GetPlayer()->m_AccData.m_Level * 10 + pChr->GetPlayer()->m_AccData.m_Health * 50 + GameServer()->m_apPlayers[BigLvlID]->m_AccData.m_Level * 5;
			pChr->MaxHealthPerRound = clamp(someint, 0, 12000); 
			pChr->IncreaseHealth(pChr->MaxHealthPerRound);
		}
		else
		{
			int someint = 200 + pChr->GetPlayer()->m_AccData.m_Level * 10 + pChr->GetPlayer()->m_AccData.m_Health * 50;
			pChr->MaxHealthPerRound = clamp(someint, 0, 12000);
			pChr->IncreaseHealth(pChr->MaxHealthPerRound);
		}
	}
	...
	{
		int someint = 700 + pChr->GetPlayer()->m_AccData.m_Level * 10 + pChr->GetPlayer()->m_AccData.m_Health * 50;
		pChr->MaxHealthPerRound = clamp(someint, 0, 12000);
		pChr->IncreaseHealth(pChr->MaxHealthPerRound);
	}	
	...
	for(int i = 0; i < 48; i++)
	{
		if(m_Door[i].m_Tick > 0)
		{
			if(m_Door[i].m_State == DOOR_CLOSED)
	 			{
				if(Server()->Tick() - 50 == m_Door[i].m_TickWhenOpening || Server()->Tick() - 100 == m_Door[i].m_TickWhenOpening ||
				   Server()->Tick() - 150 == m_Door[i].m_TickWhenOpening || Server()->Tick() - 200 == m_Door[i].m_TickWhenOpening ||
				   Server()->Tick() - 250 == m_Door[i].m_TickWhenOpening || Server()->Tick() - 300 == m_Door[i].m_TickWhenOpening ||
				   Server()->Tick() - 350 == m_Door[i].m_TickWhenOpening || Server()->Tick() - 400 == m_Door[i].m_TickWhenOpening ||
				   Server()->Tick() - 450 == m_Door[i].m_TickWhenOpening || Server()->Tick() - 500 == m_Door[i].m_TickWhenOpening ||
				   Server()->Tick() - 550 == m_Door[i].m_TickWhenOpening || Server()->Tick() - 600 == m_Door[i].m_TickWhenOpening)
				{
						m_Door[i].m_TimeToOpen--;

						for(int a = 0; a < MAX_CLIENTS; a++)
							if(GameServer()->m_apPlayers[a] && GameServer()->m_apPlayers[a]->GetCharacter())
								GameServer()->GetPlayerChar(a)->SetArmor(m_Door[i].m_TimeToOpen);
				}
				...
				if(m_Door[i].m_Tick <= Server()->Tick())
				{
					for(int a = 0; a < MAX_CLIENTS; a++)
						if(GameServer()->m_apPlayers[a] && GameServer()->m_apPlayers[a]->GetCharacter())
							GameServer()->GetPlayerChar(a)->SetArmor(0);
					SetDoorState(i, DOOR_OPEN);
					m_Door[i].m_Tick = 0;
					m_Door[i].m_TickWhenOpening = 0;
					m_Door[i].m_TimeToOpen = 10;
				}
			}
...
void IGameController::OnHoldpoint(int Index)
{
	if(m_Door[Index].m_Tick || !(m_Door[Index].m_State == DOOR_CLOSED) || !ZombStarted() || GetDoorTime(Index) == -1)
		return;
	m_Door[Index].m_Tick = Server()->Tick() + Server()->TickSpeed()*GetDoorTime(Index)-1;
	m_Door[Index].m_TickWhenOpening = Server()->Tick();
	for(int a = 0; a < MAX_CLIENTS; a++)
		if(GameServer()->m_apPlayers[a] && GameServer()->m_apPlayers[a]->GetCharacter())
			GameServer()->GetPlayerChar(a)->SetArmor(10);
}
