...
void CPlayer::Tick()
{
	...
	if (m_AccData.m_UserID && m_AccData.m_Exp >= m_AccData.m_Level)
	{
		...
		if(m_AccData.m_PlayerState == 2)
			MoneyFactor = (1*g_Config.m_SvMoneyFactorVIP);
		else if(m_AccData.m_Level < 50)
			MoneyFactor = (1*g_Config.m_SvMoneyFactorNovice);
		else MoneyFactor = (1*g_Config.m_SvMoneyFactor);

		m_AccData.m_Money += MoneyFactor;
		...
		char aBuf[64];
		str_format(aBuf, sizeof(aBuf), "'%s' got %d exp and %d money (%d now)",
			GameServer()->Server()->ClientName(m_ClientID), m_AccData.m_Exp, MoneyFactor, m_AccData.m_Money);	
		DEBUG("account", aBuf);
	}
...
void CPlayer::OnDisconnect(const char *pReason)
{
		...
		&& GameServer()->m_pController->ZombStarted() && !GameServer()->m_pController->m_Warmup)
	{
		...
		str_format(aBuf, sizeof(aBuf), "'%s' has left the game (banned 5 minutes because of last zombie)", Server()->ClientName(m_ClientID));
		GameServer()->SendChat(-1, CGameContext::CHAT_ALL, aBuf);
...
void CPlayer::SetZomb(int From)
{
	...
	if(From == -5)
	{
		...
		GameServer()->m_pController->CheckZomb();
		GameServer()->SendChatTarget(m_ClientID, "Oh... You should be more careful!"); // смерть или заражение водой
	...
	if (From > -1)
	{
		...
		GameServer()->SendChatTarget(m_ClientID, "You were infected! Do the same :)"); // заражён с молота или хаммера
	...
	if (From == -1 || From == -3)
	{
		...
		{
			char aBuf[52];			
			str_format(aBuf, sizeof(aBuf), "Look out! %s is the first zombie! 0.0", Server()->ClientName(m_ClientID));
			GameServer()->SendChat(-1, CGameContext::CHAT_ALL, aBuf);
			GameServer()->SendChat(-1, CGameContext::CHAT_ALL, "/t help to learn how to create a turret.");

			GameServer()->SendChatTarget(m_ClientID, "You are the first zombie! Infect the humans!"); // первый зомби
			GameServer()->SendChatTarget(m_ClientID, "Do mouse scroll down to throw the heart."); // первый зомби
		}
