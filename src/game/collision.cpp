...
void CCollision::MovePoint(vec2 *pInoutPos, vec2 *pInoutVel, float Elasticity)
{
	vec2 Pos = *pInoutPos;
	vec2 Vel = *pInoutVel;
...
bool CCollision::TileExists(int Index)
{
	...
	if(m_pTiles[Index].m_Index >= TILE_TELEINEVIL && m_pTiles[Index].m_Index <= TILE_SUPER_START)
		return true;
	if(m_pFront && m_pFront[Index].m_Index >= TILE_TELEINEVIL && m_pFront[Index].m_Index  <= TILE_SUPER_START)
		return true;
